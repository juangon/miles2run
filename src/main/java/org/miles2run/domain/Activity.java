...
@Entity
@NamedQueries({
        @NamedQuery(name = "Activity.findAll", query = "SELECT NEW org.miles2run.domain.ActivityDetails(a.id,a.status,a.distanceCovered,a.goalUnit,a.activityDate,a.share,a.postedBy.fullname,a.postedBy.username,a.postedBy.profilePic) FROM Activity a WHERE a.postedBy =:postedBy ORDER BY a.activityDate DESC"),
        @NamedQuery(name = "Activity.findById", query = "SELECT new org.miles2run.domain.ActivityDetails(a.id,a.status,a.distanceCovered,a.goalUnit,a.activityDate,a.share,a.postedBy.fullname,a.duration) from Activity a where a.id =:id"),
