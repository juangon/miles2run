...
import org.miles2run.domain.Activity;
import org.miles2run.domain.ActivityDetails;
import org.miles2run.domain.Profile;
import org.miles2run.domain.Progress;
import org.miles2run.exceptions.ActivityNotFoundException;
...
public class ActivityService {
    ...
    public Progress findTotalDistanceCovered(Profile profile) {
        ...
        TypedQuery<Progress> query = entityManager.createQuery("SELECT new org.miles2run.domain.Progress(a.postedBy.goal,a.postedBy.goalUnit, SUM(a.distanceCovered),COUNT(a)) from Activity a WHERE a.postedBy =:postedBy", Progress.class).setParameter("postedBy", profile);
        return query.getSingleResult();
    }
